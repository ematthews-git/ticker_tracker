...
class TestSavePricesToDb(TestPriceCollectorBase):
    ...
    def test_multiindex_columns_flattened(self, mock_execute_batch):
        ...
        df = pd.DataFrame(
            [[150.0, 155.0, 148.0, 153.0, 1_000_000]],
            index=idx, columns=col_index
        )
...
class TestFetchAndStorePrices(TestPriceCollectorBase):
    ...
    def test_calls_fetch_then_save_per_ticker(self):
        ...
        with patch.object(self.collector, '_fetch_hourly_prices',
                          return_value={'AAPL': df1, 'TSLA': df2}) as mock_fetch, \
    ...
    def test_save_error_does_not_stop_remaining_tickers(self):
        ...
        with patch.object(self.collector, '_fetch_hourly_prices',
             ...
             patch.object(self.collector, 'save_prices_to_db',
                          side_effect=[Exception("fail"), None]) as mock_save:
            # Should not raise
